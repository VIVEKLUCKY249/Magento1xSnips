...
abstract class Mage_Sales_Model_Order_Pdf_Abstract extends Varien_Object
{
    ...
    protected function insertOrder(&$page, $obj, $putOrderId = true)
    {
        ...
        if ($order->hasInvoices()) {
        	foreach ($order->getInvoiceCollection() as $invoice) {
        		...
        		$invoiceDueDate = Mage::app()->getLocale()->storeDate(
        				$order->getStore(),
        				Varien_Date::toTimestamp($invoiceDate) + (7*24*60*60),
        				true,
        				$format
        				);
        	}
     	        $page->drawText(
     	        	Mage::helper('sales')->__('Invoice Due Date: ') . Mage::helper('core')->formatDate(
     	        		$invoiceDueDate, 'medium', false
     	        	),
     	        	35,
     	        	($top -= 15),
     	        	'UTF-8'
	...
	if($order->getData('dynamics_ord') !== null):
		$page->drawText(
			Mage::helper('sales')->__('Invoice Custom Comment: ') . $order->getDynamicsOrd(),
			35,
			($top -= 15),
			'UTF-8'
		);
